...
def rotation_3d(omega, theta):
    ...
    # YOUR CODE HERE
    n_omega = np.linalg.norm(omega)
    omega_hat = skew_3d(omega)
    rot = np.identity(3) + omega_hat/n_omega * np.sin(n_omega*theta) + (omega_hat@omega_hat)/(n_omega**2) * (1 - np.cos(n_omega*theta))
    return rot
...
def hat_3d(xi):
    ...
    # YOUR CODE HERE
    xi_hat = np.hstack((skew_3d(xi[3:]), np.transpose([xi[:3]])))
...
def homog_3d(xi, theta):
    ...
    if not omega.any():
        g = np.hstack((np.identity(3), np.transpose([nu]) * theta))
        g = np.vstack((g, np.array([[0, 0, 0, 1]])))
    else:
        rot_ = rotation_3d(omega, theta)
        ur_ele = 1/(np.linalg.norm(omega)**2) \
                * ((np.identity(3) - rot_) @ (skew_3d(omega) @ np.transpose([nu])) \
                    + np.transpose([omega]) @ (np.transpose([omega])).T @ np.transpose([nu]) * theta)   # Upper-right element
        g = np.hstack((rot_, ur_ele))
        g = np.vstack((g, np.array([[0, 0, 0, 1]])))
    return g
